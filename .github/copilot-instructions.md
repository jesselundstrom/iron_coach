...
- The current shell-replacement step runs through a single React shell entry point in `src/app-shell/main.jsx`: visible bottom navigation, toast host, confirm modal, exercise catalog/name modal, workout overlay hosts, settings overlay hosts, and the top-level page container (rendered into the existing content root via portal). `core/ui-shell.js` remains the compatibility bridge for `showPage(...)`, `showToast(...)`, `showConfirm(...)`, page activation side effects, and legacy DOM ids.
